...
export default {
  ...
  setup() {
    ...
    let nextId = 1;
    const numbers = ref([1965, 2003, 2022, 1984, 1945, 1999, 2001, 1977, 1953].sort(() => 0.5 - Math.random()));
    ...
    function dotClick(index) {
      if (!dotUsed.value && randomNumber.value) {
        const leftNumber = index > 0 && dots.value[index - 1]?.type === 'number' ? dots.value[index - 1].number : null;
        const rightNumber = index < dots.value.length - 1 && dots.value[index + 1]?.type === 'number' ? dots.value[index + 1].number : null;

        if (isValidNumber(randomNumber.value, leftNumber, rightNumber)) {
          dots.value.splice(index, 1, { id: nextId++, type: 'number', number: randomNumber.value });
